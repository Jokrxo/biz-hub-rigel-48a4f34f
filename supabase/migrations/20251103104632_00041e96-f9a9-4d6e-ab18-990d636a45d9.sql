-- Align transaction_entries.status with its CHECK constraint
-- Change from 'approved' to 'pending'
...
BEGIN
  ...
  -- Use status that passes CHECK constraint: pending | allocated | deleted
  INSERT INTO transaction_entries (transaction_id, account_id, debit, credit, description, status)
  VALUES
    (v_transaction_id, v_bank_account_id, NEW.opening_balance, 0, 'Opening Balance - Debit', 'pending'),
    (v_transaction_id, v_opening_balance_equity_id, 0, NEW.opening_balance, 'Opening Balance - Credit', 'pending');
