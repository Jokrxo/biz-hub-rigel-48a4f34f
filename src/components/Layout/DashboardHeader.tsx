...
export const DashboardHeader = ({ onMenuClick }: DashboardHeaderProps) => {
  ...
  // Supabase connection state (single declaration)
  // (status, latency, strength)
  ...
  return (
    <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 px-4 sm:px-6 shadow-sm">
      ...
      <div className="flex items-center gap-3 sm:gap-4">
        ...
        <div className="hidden md:flex items-center gap-2 bg-background p-1.5 rounded-md border shadow-sm">
          ...
          <div className="text-xs">
            {demoMode ? 'Demo Mode' : sbStatus === 'online' ? 'Supabase Online' : sbStatus === 'connecting' ? 'Connecting…' : 'Supabase Offline'}
          </div>
          {sbLatency !== null && (
